(function Latinify() {
    if (!(Spicetify.Player?.data && Spicetify.Menu && Spicetify.LocalStorage && Spicetify.Platform)) {
        setTimeout(Latinify, 300);
        return;
    }
    InitLatinify();
})();

function InitLatinify() {
    console.log("%cLatinify", "color: #888ebf; font-size: 5em", "Made with <3 by LucyUwi")

    //////// Main Functions ////////

    class Latinify {

        //////// Convert all Cyrillic Character to Latin ones ////////
        ...
        static ToggleTrans() {
            if (localStorage.getItem("Trans") == "false") {
                ...
                document.getElementById("ToLatin").classList.add("main-genericButton-buttonActive")
                document.getElementById("ToLatin").classList.add("control-button--active")
            } else {
                localStorage.setItem("Trans",false)
                document.getElementById("ToLatin").children[0].children[0].setAttribute("fill","currentColor")
                document.getElementById("ToLatin").classList.remove("control-button--active")
                document.getElementById("ToLatin").classList.remove("main-genericButton-buttonActive")
            }
        }
        ...
        static CreateButton() {
            let ToLatin = document.createElement("button")
            ...
            ToLatin.addEventListener("click", function () { Latinify.ToggleTrans(); }, false);
            ToLatin.setAttribute("class", "main-genericButton-button control-button")
            ToLatin.setAttribute("id", "ToLatin")
            ToLatin.setAttribute("title", "Latinify");

            let ExtraButtons = document.querySelector(".main-nowPlayingBar-right")?.childNodes[0]
            ExtraButtons?.prepend(ToLatin)
        ...
        static Activate() {
            if (localStorage.getItem("Trans") == null){
                ...
                if (!TransButton) {
                    setTimeout(() => {
                        TransButton = document.getElementById("ToTrans")
                    }, 500)
                }
                TransButton.children[0].children[0].setAttribute("fill","var(--spice-button)")
            }
        }
    }

    //////// Call for Button to be Created ////////

    Latinify.CreateButton()
